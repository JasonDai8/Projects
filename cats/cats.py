...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def checker(some_string):
        clean_string = remove_punctuation(some_string)
        lower_string = clean_string.lower()
        word_list = split(lower_string)
        for i in topic:
            for j in word_list:
                if i == j:
                    return True
...
def accuracy(typed, reference):
    ...
    if len(typed_words) == 0 and len(reference_words) == 0:
        ...
        return 0.0
    shortest = min(len(typed_words), len(reference_words))
    status = False
    if shortest == len(reference_words):
        status = True
    elif shortest == len(typed_words):
        status = False
    for i in range(0, shortest):
        ...
        denominator += 1
    if status:
        denominator = denominator + (len(typed_words)-len(reference_words))
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for i in range(0, len(valid_words)):
        ...
        current_diff = diff_function(w1,valid_words[i],limit)
        new_pair = [i,current_diff]
        holder.append(new_pair)
    smallest_pair = min(holder, key= lambda item: item[1])
    if smallest_pair[1] > limit:
        return w1
    return valid_words[smallest_pair[0]]
    # END PROBLEM 5
...
def time_per_word(times_per_player, words):
    ...
    for player in times_per_player:
        ...
        for i in range(0, len(player)-1):
            new_list.append(player[i+1]-player[i])
        specific_times.append(new_list)
    return game(words, specific_times)
    # END PROBLEM 9
        ...
        for i in word_indices:
            ...
            for j in player_indices:
                if time(game,j,i) < best_time:
                    ...
                    best_player = j
            if best_time == time(game,player,i) and best_player == player:
                players_words.append(word_at(game,i))
        all_players_records.append(players_words)
