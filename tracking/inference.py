...
def constructBayesNet(gameState: hunters.GameState):
    ...
    observedDist = list(range(0, X_RANGE + Y_RANGE + MAX_NOISE - 1))

    variableDomainsDict[OBS0] = observedDist
    variableDomainsDict[OBS1] = observedDist
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        bayesNetEvidence = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        fList = list(bayesNetEvidence)
        
        for i in eliminationOrder:
            fList, joinedFactor = joinFactorsByVariable(fList, i)
            uncondVarJoinedFac = joinedFactor.unconditionedVariables()

            if {i}.issubset(uncondVarJoinedFac):
                lengthUncondVar = len(joinedFactor.unconditionedVariables())
                if lengthUncondVar > 1:
                    joinedFactor = eliminate(joinedFactor, i)
                    fList.append(joinedFactor)
                else:
                    pass
        norm = normalize(joinFactors(fList))
        return norm
        "*** END YOUR CODE HERE ***"
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        sample = self.total()*random.random() 
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            ...
            busterProb = busters.getObservationProbability(noisyDistance, dist)
            return busterProb
        else:
            return 0
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(self.numParticles):
            ...
            if c >= numPos:
                c = 0
            legalpos = self.legalPositions[c]
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            belief[particle] = self.particles.count(particle)
        belief.normalize()
        return belief
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            distribution[particle] = distribution[particle] + self.getObservationProb(observation, pacPos, particle, jailPos)
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        for i in range(self.numParticles):
            newDist = self.getPositionDistribution(gameState, self.particles[i])
