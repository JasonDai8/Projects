...
def sentence1() -> Expr:
    ...
    a_or_b = ~A % (~B | C)
    triple_a = logic.disjoin(A,B)
    not_a = logic.disjoin(~A,~B,C)
    return logic.conjoin(triple_a,a_or_b,not_a)
...
def sentence2() -> Expr:
    ...
    not_d = ~D >> C
    triple_a = C % (B | D)
    return logic.conjoin(triple_a,a_or_b,not_a,not_d)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    pacalive0 = PropSymbolExpr('PacmanAlive', time=0)
    pacalive1 = PropSymbolExpr('PacmanAlive', time=1)
    pacborn = PropSymbolExpr('PacmanBorn', time=0)
    packilled = PropSymbolExpr('PacmanKilled', time=0)
    
    c1 = pacalive1 % ((pacalive0 & ~packilled) | (~pacalive0 & pacborn))
    c2 = ~(pacalive0 & pacborn)
    c3 = pacborn
