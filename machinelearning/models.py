...
class PerceptronModel(object):
    ...
    def get_prediction(self, x):
        ...
        return (-1, 1)[nn.as_scalar(self.run(x))>=0]
    ...
    def train(self, dataset):
        ...
        while not condition:
            ...
            for i, j in dataset.iterate_once(1):
                ...
                actual = nn.as_scalar(j)
                
                if prediction != actual:
                    condition = False
                    wself = self.w
                    wself.update(i, actual)
...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        mult0 = nn.Linear(x, self.w1)
        sum0 = nn.AddBias(mult0, self.b1)
        y1 = nn.ReLU(sum0)

        mult1 = nn.Linear(y1, self.w2)
        sum1 = nn.AddBias(mult1, self.b2)
        if self.numHiddenLayers == 1:
            return sum1
        y2 = nn.ReLU(sum1)

        mult2 = nn.Linear(y2, self.w3)
        sum2= nn.AddBias(mult2, self.b3)
        if self.numHiddenLayers == 2:
            return sum2
        y3 = nn.ReLU(sum2)

        mult3 = nn.Linear(y3, self.w4)
        sum3 = nn.AddBias(mult3, self.b4)
    ...
    def train(self, dataset):
        ...
        if self.numHiddenLayers == 1:
            alist = [self.w1, self.b1, self.w2, self.b2]
        elif self.numHiddenLayers == 2:
            alist = [self.w1, self.b1, self.w2, self.b2, self.w3, self.b3]
        else:
            alist = [self.w1, self.b1, self.w2, self.b2, self.w3, self.b3, self.w4, self.b4]

        for i in range(500):
            for j, k in dataset.iterate_once(self.batchSize):
                loss = self.get_loss(j, k)
                grad = nn.gradients(loss, alist)
...
class DigitClassificationModel(object):
    ...
    def train(self, dataset):
        ...
        for i, j in dataset.iterate_forever(self.batchSize):
